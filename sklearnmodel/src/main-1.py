...
def ModelPipe(args):
    ...
    model = Pipeline(steps=[
        ('drop',ml_custom.ExtractColumn(columns=['MedInc', 'HouseAge', 'AveRooms', 'AveBedrms', 'Population', 'AveOccup',
       'Latitude', 'Longitude'])),
        ('scaler',ml_custom.CustomStandardScaler(['MedInc', 'HouseAge', 'AveRooms', 'AveBedrms', 'Population', 'AveOccup',
       'Latitude', 'Longitude'])),
        ('model',RandomForestRegressor(n_estimators=args.n_estimators,max_depth=10,n_jobs=-1))
    ])
    return model
